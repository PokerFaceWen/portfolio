...
- [x] 为阿里云部署补一条自动化流水线，避免 GitHub Pages 和自建站内容不同步。
- [x] 将生产部署拆成 `build`、`pages deploy`、`server deploy` 三段，分别记录状态和失败原因。
- [x] 给阿里云部署增加 `rsync --delete` 或原子发布目录切换，避免残留旧文件。
- [x] 为构建产物增加一次部署前检查，确认 `base`、favicon、canonical、静态资源路径符合预期。
- [x] 在仓库中补充一份"切换根路径部署"的操作手册，明确 `astro.config.ts`、Nginx rewrite、站点链接需要同时调整。

## 二、服务器安全整改

- [x] 配置 SSH 密钥登录，并验证可用后禁用密码登录。
